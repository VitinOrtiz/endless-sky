...
void Ship::Load(const DataNode &node, const ConditionsStore *playerConditions)
{
	...
	for(const DataNode &child : node)
	{
		...
		else if((key == "engine" || key == "reverse engine" || key == "steering engine") && child.Size() >= 3)
		{
			...
			if(reverse)
				engine.SetFacing(*(new Angle(180.)));
			for(const DataNode &grand : child)
			{
				const string &grandKey = grand.Token(0);
				bool grandHasValue = grand.Size() >= 2;
				if(grandKey == "zoom" && grandHasValue)
					engine.SetZoom(grand.Value(1));
				else if(grandKey == "angle" && grandHasValue)
					engine.SetFacing( engine.Facing() + Angle(grand.Value(1)) );
				else if(grandKey == "gimbal" && grandHasValue)
					engine.SetGimbal(engine.Gimbal() + Angle(grand.Value(1)));
				else
				{
		...
		else if(key == "outfits")
		{
			...
			crew.Add(static_cast<int>(child.Value(1)));
...
vector<string> Ship::FlightCheck() const
{
	...
	if(checks.empty())
	{
		if(crew.RequiredCrew() > crew.Bunks())
...
bool Ship::CanSendHail(const PlayerInfo &player, bool allowUntranslated) const
{
	...
	if(CannotAct(Ship::ActionType::COMMUNICATION) || !Crew().Members() || GetPersonality().IsMute() || GetPersonality().IsQuiet())
		return false;
...
void Ship::Launch(list<shared_ptr<Ship>> &ships, vector<Visual> &visuals)
{
	...
	if(!IsDestroyed() && (!commands.Has(Command::DEPLOY) || zoom != 1.f || hyperspaceCount ||
	...
	{
		bay.Launch(visuals);
		ships.push_back(bay.Fighter()->shared_from_this());
	}
}
...
shared_ptr<Ship> Ship::Board(bool autoPlunder, bool nonDocking)
{
	...
	if(!government->IsEnemy(victim->GetGovernment()))
	{
		...
		if(helped)
		{
			GetCrew().SetPilotError(120);
			victim->GetCrew().SetPilotError(120);
		}
		return victim;
	}
...
bool Ship::IsDisabled() const
{
	...
	bool needsCrew = GetCrew().RequiredCrew() != 0;
	return (hull < minimumHull || (!GetCrew().Members() && needsCrew));
}
...
{
	bool cannotAct = zoom != 1.f || isDisabled || hyperspaceCount || GetCrew().PilotError() ||
		(actionType == ActionType::COMMUNICATION && !GetCrew().Members());
	if(cannotAct)
		return true;
...
const vector<EnginePoint> &Ship::EnginePoints() const
{
	return enginePoints;
}
...
const vector<EnginePoint> &Ship::ReverseEnginePoints() const
{
...
const vector<EnginePoint> &Ship::SteeringEnginePoints() const
{
...
int Ship::WasCaptured(const shared_ptr<Ship> &capturer)
{
	...
	//	AddCrew(transfer);
	//}
	int transfer = GetCrew().WasCaptured(capturer);
...
{
	return GetCrew().RequiredCrew() && GetCrew().Members() && !IsDisabled();
}
...
const vector<Bay> &Ship::Bays() const
{
	return bays;
}
...
void Ship::DoGeneration()
{
	...
	if(!isDisabled)
	{
		...
		if(!bays.empty())
		{
			...
			for(const Bay &bay : bays)
				if(bay.Fighter())
					carried.emplace_back(1. - bay.Fighter()->Health(), bay.Fighter());
	...
	isDisabled = isOverheated || hull < MinimumHull() || (!GetCrew().Members() && GetCrew().RequiredCrew());
...
//	}
//	else if(requiredCrew && static_cast<int>(Random::Int(requiredCrew)) >= GetCrew().Members())
//	{
//		pilotError = 30;
...
void Ship::StepTargeting()
{
	...
	if(target && !isDisabled)
	{
		...
		if(isBoarding && !GetCrew().PilotError())
		{
			Angle facing = angle;
...
double Ship::CarriedMass() const {
	return carriedMass;
}

void Ship::AddCarriedMass(double mass) {
	carriedMass += mass;
}
