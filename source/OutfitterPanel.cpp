...
void OutfitterPanel::Buy(bool onlyOwned)
{
	...
	for(int i = 0; i < modifier && CanBuy(onlyOwned); ++i)
	{
		...
		for(Ship *ship : shipsToOutfit)
		{
			...
			if(required && ship->GetCrew().Members() + required <= static_cast<int>(ship->GetCrew().Bunks()))
				ship->GetCrew().Add(required);
			ship->Recharge();
		}
	}
}
	...
	if(!shipsToOutfit.empty())
	{
		for(Ship *ship : shipsToOutfit)
		{
			...
			if(selectedOutfit->Get("required crew"))
				ship->GetCrew().Add(-selectedOutfit->Get("required crew"));
			ship->Recharge();
