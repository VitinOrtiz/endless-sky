...
void HiringPanel::Draw()
{
	...
	if(flagship)
	{
		...
		flagshipRequired = flagship->GetCrew().RequiredCrew();
		flagshipExtra = flagship->GetCrew().Members() - flagshipRequired;
		flagshipUnused = flagshipBunks - flagship->GetCrew().Members();
	}
	...
	for(const shared_ptr<Ship> &ship : player.Ships())
		if(!ship->IsParked())
		{
			...
			fleetRequired += ship->GetCrew().RequiredCrew();
		}
...
bool HiringPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	{
		player.Flagship()->GetCrew().Add(min(maxHire, Modifier()));
		player.UpdateCargoCapacities();
	}
	else if(key == 'f' || key == SDLK_MINUS || key == SDLK_KP_MINUS || key == SDLK_BACKSPACE || key == SDLK_DELETE)
	{
		player.Flagship()->GetCrew().Add(-min(maxFire, Modifier()));
		player.UpdateCargoCapacities();
	}
	else
		return false;
