...
void BoardingPanel::Draw()
{
	...
	if(CanAttack() && (you->GetCrew().Members() > 1 || !victim->GetCrew().RequiredCrew()))
	...
	int vCrew = victim ? victim->GetCrew().Members() : 0;
...
bool BoardingPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	{
		int yourStartCrew = you->GetCrew().Members();
		int enemyStartCrew = victim->GetCrew().Members();
		...
		bool youAttack = (key == 'a' && (yourStartCrew > 1 || !victim->GetCrew().RequiredCrew()));
		if(key == 'a' && !youAttack)
			return true;
		...
		else
		{
			...
			{
				int yourCrew = you->GetCrew().Members();
				int enemyCrew = victim->GetCrew().Members();
				...
				if(youAttack)
				{
					...
					if(total)
					{
						if(Random::Real() * total >= yourAttackPower)
						{
							++yourCasualties;
							you->GetCrew().Add(-1);
							if(you->GetCrew().Members() <= 1)
								break;
						}
						else
						{
							++enemyCasualties;
							victim->GetCrew().Add(-1);
							if(!victim->GetCrew().Members())
								break;
						}
					}
				}
			...
			else if(!victim->GetCrew().Members())
			{
				messages.push_back("You have succeeded in capturing this ship.");
				victim->GetGovernment()->Offend(ShipEvent::CAPTURE, victim->GetCrew().ReputationalValue());
				...
				for(const Bay &bay : victim->Bays())
					if(bay.Fighter())
					{
						player.AddShip(bay.Fighter()->shared_from_this());
						player.HandleEvent(ShipEvent(you, bay.Fighter()->shared_from_this(), ShipEvent::CAPTURE), GetUI());
					}
				isCapturing = false;
...
bool BoardingPanel::CanCapture() const
{
	...
	return (!victim->GetCrew().RequiredCrew() || you->GetCrew().Members() > 1);
}
